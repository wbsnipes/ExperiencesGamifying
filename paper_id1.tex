...
As software development practices evolve, toolsmiths face the continuous challenge of communicating new practices and tools to the community.   We tested an idea with industrial software developers that adding game-like feedback to the development environment focused on improving practices and tool use would improve adoption of tools and practices for code navigation.  We conducted this study in three stages, the pre-study to validate our ideas, a pilot of a tool called Blaze that implements some game features to motivate better navigation practices and use of a code search tool.  We found that developers used the tool willingly and their feedback indicated the Blaze tool and game made them think about navigation practices more.  We also learned that demonstrating significant improvement can be difficult and that developers want more proactive messages and recommendations on ways they can improve.
%
...
\item Developers favor a team-oriented competition competition over individual awards
...
\end{itemize}
Thus, this pre-study survey provided a strong positive indicator for going forward with a software engineering tool that focused on sharing and rewarding developers for using good practices and tools. 
\section{Game Design}
...
Following the pre-study results, we began implement a game-like tool for developers to use in Visual Studio.  We defined the goals for the tool and designed the game elements that it would include.  Here we discuss  relevant aspects of selecting a practice suitable for gamification, designing the game, and competitive element considerations.

\subsection{Select a Practice}
The first step is to select a development practice to incorporate into the game.  
Selecting a practice to study involves identifying a practice, defining specific characteristics of the practice to measure, and identifying the actions in Visual Studio that relate to the practice.  In their paper on game design patterns, Hamari et. al. issue a warning that assigning achievements to required tasks reduces intrinsic motivation because player autonomy is reduced by the achievements \cite{wbsnipes:Hamari2011Framework} .  Thus we avoided giving points for actions like fixing bugs or completing tasks.  We also currently limit ourselves to practices that exist entirely in the IDE thus can be monitored using Blaze.  We identified potential practices of test-driven development, debugging practices, navigation practices,  eliminating static analysis bugs, and check-in frequency.  
...
We categorize commands as structured navigation when they allow the developer to locate a code element following the program structure from another element.  These commands include Navigate To \textit{Ctrl+,}, Go To Definition \textit{F12}, and Find All References \textit{Ctrl+K,R} that are built-in to Visual Studio and follow the structured graph of the code.
...
%  This contradicts a result from the pre-study survey where respondents rated badges as less important than team or individual competition in motivating them to improve.  The important difference in this case is badges serve as both a feedback mechanism and information showing which commands are important to use for improving structured navigation and their score.
...
%We know that not all participants remained in the study or continued using Blaze, however, our conclusions are based mostly on those that continued with the study.   
