...
After classifying the commands we assigned points to the commands in the same configuration file.  Structured navigation commands received 1-10 points each while unstructured navigation commands received 0 points.  Assigning negative points while possible, is discouraged as this may demotivate folks who do not have detailed knowledge of how the study is designed.  When assigning points, make sure you recognize the relationship between points and levels such that "leveling up" happens at a frequent enough pace to make that motivating for users during the study.  Ideally users would level up after a working day or two worth of activity initially extending to a week or even a month following an exponential curve \cite{gamification}.  Designing the points and levels such that above average developers can complete all levels during the study period is ideal.

\subsection{Study Intervention and Participants}
Part of the study can include a communication mechanism that informs the users about the study or even provides training relevant to the objective of the study via a web site.  We discuss this as the "intervention" that expresses the practice that we wish developers to adopt.  For this study we chose to make a nice looking graphic that users click on to see information about navigation commands, install the Sando search tool, and learn about the Blaze game.  The site uses a combination of video and blog-post style information easily accessible to the developer.  The communication site is rated as part of our study completion survey.

Timing the study communication is part of the consideration for a good study design.  We chose to deploy the Blaze tool without communicating the study purpose for a baseline period, then the tool would pop-up a display with the information on the communication site asking the developers to consume the information and begin using the practices.   This allows us to measure changes in the developer's practices over time during the study.  %Other intervention approaches may be used, however, you should take care to understand how the method and timing may affect the measurement of the practices.

We conducted this study with developers working in an R\&D facility in India on a large industrial software system.  Developers were asked to volunteer for the pilot by their management, however, managers did not know who participated in the study.  Keeping managers out of the loop is intended to reduce concerns among the developers that the data may be used for purposes other then the intended study.  By selecting an intact team, we hope to leverage aspects of motivation such as the Leaderboard to spur feelings of competition among teammates.  
...
\balance
\bibliography{paper_id1}  % sigproc.bib is the name of the Bibliography in this case
