...
Blaze helps understand developer practice and tool use over the long term and judges the lasting change a communication effort makes in everyday habits.  Embeding Blaze n the developers work environment as a Visual Studio extension provides the capture of usage data while giving feedback to users on their practices.  Blaze encourages everyone to use the new practices by assigning points to specific commands and tools .  Providing a light score ranking view should help us stay motivated as we compare our command usage to our peers.  A month-long longitudinal study conducted with pilot users shows how develipers react to the presented information.    

Results from this demonstrative study show the approach is <blank> with a few developers <blank> of the study.  The effectiveness of this approach for communicating about practices is demonstrated through the continued use of the commands as we communicate them.  Evaluation of metrics for navigation ratio over time show that developer use the commands <blank> than they did during the benchmarking phase.  The result of the study shows that this method for communicating and promoting software engineering tools and practices <blank>.
\end{abstract}
...
%  Our post pilot survey attempts to shed some light on the reasons behind the inconsistent improvement in navigation ratio.    Even developers whose navigation ratio  was lower than average, used structured navigation occasionally as much as their more frequently using colleagues.  The variation for each individual can be seen in Figure \ref{} .
